    ...
    while True:
        if select.select([sys.stdin], [], [], 0) == ([sys.stdin], [], []):
            ...
            if c == '\x1b':  # Esc
                ...
                print("Enter receiver address for CPU temperature updates (0-255):", end='', flush=True)
                addr_input = input()
                try:
                    send_to_who = int(addr_input)
                    if 0 <= send_to_who <= 255:
                ...
                send_cpu_continue(node, 30, False)

...
def set_addresses():
    print("Set sender and receiver addresses.")
    print("Enter sender address:", end='', flush=True)
    sender_addr = int(input())
    print("Enter receiver address:", end='', flush=True)
    receiver_addr = int(input())
    return sender_addr, receiver_addr

def send_deal(sender_addr, receiver_addr):
    get_rec = ""
    print("")
    print(f"Sender address is {sender_addr}. Enter a string such as \033[1;32m{receiver_addr},Hello World\033[0m, it will send `Hello World` to node of address {receiver_addr}.", flush=True)
...
def send_cpu_continue(sender_addr, receiver_addr, continue_or_not=True):
    if continue_or_not:
        global timer_task
        global seconds
        node.send_to = receiver_addr
        node.addr_temp = node.addr
        node.set(node.freq, node.send_to, node.power, node.rssi)
        node.send("CPU Temperature:" + str(get_cpu_temp()) + " C")
        time.sleep(0.2)
        node.set(node.freq, node.addr_temp, node.power, node.rssi)
        timer_task = Timer(seconds, send_cpu_continue, (sender_addr, receiver_addr,))
        timer_task.start()
    else:
        node.send_to = receiver_addr
        node.addr_temp = node.addr
...
try:
    ...
    sender_addr, receiver_addr = set_addresses()
    ...
    while True:
        if select.select([sys.stdin], [], [], 0) == ([sys.stdin], [], []):
            ...
            # detect key i
                ...
                send_deal(sender_addr, receiver_addr)
            # detect key s
            elif c == '\x73':
            ...
            # detect key r to receive data
                ...
                send_cpu_continue(sender_addr, receiver_addr, False)
